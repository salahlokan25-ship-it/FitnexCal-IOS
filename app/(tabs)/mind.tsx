...
export default function MindScreen() {
  ...
  const stopRecordingNative = useCallback(async () => {
    ...
    try {
      ...
      setIsRecording(false);
      console.log('[STT] Native recording stopped, uri:', uri);
      ...
      if (!text) {
        setSttError('Could not understand the audio. Please try again or type a short description.');
        return;
      }

      setTranscript(text);
    } catch (e) {
      console.log('[STT] native stop/upload error', e);
      setSttError('Upload failed');
    }
  }, [nativeRecording]);
      ...
      <ScrollView style={dynamic.scrollView} showsVerticalScrollIndicator={false} testID="mind-scroll">
        ...
        <AnimatedFadeIn delay={140}>
          ...
          >
            ...
            <Text style={dynamic.tileSubtitle}>
              {Platform.OS === 'web'
                ? 'Press to record by voice.'
                : 'Open and use the keyboard mic, then analyze.'}
            </Text>
            <View style={dynamic.roundIconPrimary}>
        ...
        <TouchableOpacity style={dynamic.modalOverlay} activeOpacity={1} onPress={() => setShowVoiceModal(false)}>
          <TouchableOpacity style={dynamic.modalContent} activeOpacity={1} onPress={(e) => e.stopPropagation()}>
            ...
            >
              ...
              <Text style={dynamic.modalLabel}>
                {Platform.OS === 'web'
                  ? 'Describe what you ate'
                  : 'Describe what you ate (use the keyboard mic to dictate)'}
              </Text>
              <TextInput
                ...
                testID="voice-input"
              />
              {sttError && <Text style={dynamic.errorText}>{sttError}</Text>}
