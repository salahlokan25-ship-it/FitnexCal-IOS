...
export default function FoodAnalysisScreen() {
  ...
  const saveMealsToType = async (mealType: MealEntry['meal_type']) => {
    setIsAdding(true);
    try {
      for (let i = 0; i < analysis.foods.length; i++) {
        ...
        await addMeal(foodItem, quantity, mealType, imageUri as string);
      }
  ...
  const handleAddToLog = () => {
    if (Platform.OS === 'ios') {
      ActionSheetIOS.showActionSheetWithOptions(
        {
          title: 'Save scanned food to',
          options: ['Cancel', 'Breakfast', 'Lunch', 'Dinner', 'Snack'],
          cancelButtonIndex: 0,
        },
        (buttonIndex) => {
          if (buttonIndex === 0) return;
          const map: Record<number, MealEntry['meal_type']> = {
            1: 'breakfast',
            2: 'lunch',
            3: 'dinner',
            4: 'snack',
          };
          const chosen = map[buttonIndex] ?? 'breakfast';
          setSelectedMealType(chosen);
          void saveMealsToType(chosen);
        },
      );
    } else {
      // Fallback: use current selectedMealType on non‑iOS platforms
      void saveMealsToType(selectedMealType);
    }
  };
